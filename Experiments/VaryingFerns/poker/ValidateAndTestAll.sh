...
do
  ...
  do
    ...
    do
      ...
      line=`"${toolCmd}" test -n 6253 -g ../../Validation.sad -I .. ${toolFlags} -w "${weightsFile}" | grep "running average" | tail -n 1`
      loss=`echo "${line}" | awk '{ gsub("\r",""); print $NF }'`

      #line=`"${toolCmd}" test -n 500 -g ../validation.sad -w "${weightsFile}" | grep "running accuracy" | tail -n 1`
    ...
    line=`"${toolCmd}" test -n 1000000 -g ../../Test.sad -I .. ${toolFlags} -w "${bestWeightsFile}" | grep "running accuracy" | tail -n 1`
