...
do
  ...
  do
    ...
    do
      ...
      line=`"${toolCmd}" test -n 50 -g ../../Validation.sad -I . ${toolFlags} -w "${weightsFile}" | grep "running average" | tail -n 1`
    ...
    line=`"${toolCmd}" test -n 50 -g ../../Test.sad -I . ${toolFlags} -w "${bestWeightsFile}" | grep "running accuracy" | tail -n 1`
